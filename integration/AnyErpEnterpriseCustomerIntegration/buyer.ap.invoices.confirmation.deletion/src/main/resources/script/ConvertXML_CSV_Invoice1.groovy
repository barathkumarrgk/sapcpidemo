...
def Message processData(Message message) {
    ...
    def headerNames = messages."${xmlRecordPrefix}".root[0].notifications[0].children().collect { it.name() }
    ...
    // Gather CSV Rows
//    messages."${xmlRecordPrefix}".root.each { notifications ->
//        def rowData = notifications.children().collect { it.text() }
